...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    // We request RGB565 because hardware JPEG encoder on v1.3 does not support YUV420
    ...
    struct v4l2_requestbuffers reqbuf = { .count = 2, .type = type, .memory = V4L2_MEMORY_MMAP };
    ioctl(fd, VIDIOC_REQBUFS, &reqbuf);

    // --- Set Manual Exposure and Gain for indoor lighting ---
    struct v4l2_query_ext_ctrl q_exp = { .id = V4L2_CID_EXPOSURE };
    if (ioctl(fd, VIDIOC_QUERY_EXT_CTRL, &q_exp) == 0) {
        struct v4l2_ext_control c_exp = { .id = V4L2_CID_EXPOSURE, .value = q_exp.maximum - (q_exp.maximum / 4) }; // 75% of max
        struct v4l2_ext_controls cs_exp = { .ctrl_class = V4L2_CTRL_CLASS_USER, .count = 1, .controls = &c_exp };
        ioctl(fd, VIDIOC_S_EXT_CTRLS, &cs_exp);
        ESP_LOGI(TAG, "Set Manual Exposure to %d (Max: %lld)", c_exp.value, (long long)q_exp.maximum);
    }

    struct v4l2_querymenu q_gain = { .id = V4L2_CID_GAIN, .index = 10 }; // Try index 10 or find max
    int max_gain_idx = 0;
    ...
    if (max_gain_idx > 0) {
        struct v4l2_ext_control c_gain = { .id = V4L2_CID_GAIN, .value = max_gain_idx / 2 }; // 50% gain
        struct v4l2_ext_controls cs_gain = { .ctrl_class = V4L2_CTRL_CLASS_USER, .count = 1, .controls = &c_gain };
        ioctl(fd, VIDIOC_S_EXT_CTRLS, &cs_gain);
        ESP_LOGI(TAG, "Set Manual Gain Index to %d (Max: %d)", c_gain.value, max_gain_idx);
    }
    // --------------------------------------------------------
    ...
    while (true) {
        ...
        uint32_t out_len = 0;
        esp_err_t enc_ret = jpeg_encoder_process(enc, &cfg, buffer[buf.index], w * h * 2, out, out_alloc, &out_len);
